...
import io.github.firstred.iptvproxy.m3u.M3uDoc;
                ...
                m3u.getChannels().forEach((c) -> {
                    ...
                    if (serverChannel == null) {
                        try {
                            ...
                            throw new RuntimeException("error creating server channel", e);
                        }
                    }

                    channel.addServerChannel(serverChannel);
                    LOG.info("Add server channel for channel: {}, url: {}", c.getName(), url);
